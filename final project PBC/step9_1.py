...
def check_hit_things():
    ...
    ## 因為要利用這兩個函式的結果當作條件判斷式，因此hit_walls()、hit_body()的return值型別一定是bool
    ## 當hit_walls()函式return True時，表示有撞到牆；return False時表示沒撞到牆
    ## 因此是否有「撞牆」可用hit_walls()來表示
    ##
    ## 當hit_body()函式return True時，表示有撞到自己；return False時表示沒撞到自己
    ## 因此是否有「撞自己」可用hit_walls()來表示
    ##
    ## 還要用hit_food()函式來當作是否有撞到食物的依據
    ## 當hit_food()函式return True時，表示有撞到食物；return False時表示沒撞到食物
    ## 因此是否有「撞食物」可用hit_food()來表示
    ...
    ## 大家可以想想body_grow()跟gen_food()的先後順序為何？為什麼？
        ??
        ??
    else:
        pass
