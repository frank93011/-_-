...
## game_exit：用來標記視窗關閉是否觸發了，當game_exit為True代表視窗的x被點擊
##            初始狀態應設為? True? False?
game_exit = False
def root_exit():
    '''
...
def loop():
    ...
    ## 計時器，從0數到timer_reset(即60000)
    ## 沒有按入方向鍵的情況下，每次計時器數到60000時，就讓蛇照著目前的行進方向前進一格
    ...
    while not game_exit:
        ...
        ## 或是方向沒改變，所以等計時器倒數結束蛇才前進(timer == timer_reset)
            ...
            ## 蛇移動後，dir_changed應該要設回False
            ## 讓下一個按壓方向鍵的事件來決定是否有改變方向的情形
            dir_changed = ??
