...
class snake_window:
    ...
    def add_super_score(self):
        ...
        return
    def add_lottery_score(self):
        self.score += self.score_step*10
        return
    def delete_score(self):
        self.score -= self.score_step
        return
    def delete_lottery_score(self):
        self.score -= self.score_step*10
        return
    ...
    def add_super_speed(self):
        ...
        self.timer_count -= int(self.timer_init * self.speed_step*2 / 10)
        return
    ...
    def move(self):
        ...
        if self.snake.hit_walls() or self.snake.hit_body():
            ...
            #self.snake.gen_strange()
        elif self.snake.hit_lottery():
            self.lstart =time.time()
            z = random.randint(10,20)
            w = random.randint(0,1)
            if w == 0:
                self.snake.body_add10()
                self.add_lottery_score()
            else:
                self.snake.body_delete_10()
                self.delete_lottery_score()
            self.snake.canvas.delete(self.snake.lottery_rect)
            self.root.after(z*1000,self.snake.gen_lottery)
...
class snake:
    ...
    def gen_lottery(self):           
        ...
        while self.lottery in self.cstack:
            x = random.randint(0, self.width - 1)
            y = random.randint(0, self.height - 1)
            self.lottery = (x, y)
        if self.lottery == self.food:
            x = random.randint(0, self.width - 1)
            y = random.randint(0, self.height - 1)
            self.lottery = (x, y)
        elif self.lottery == self.strange:
            x = random.randint(0, self.width - 1)
            y = random.randint(0, self.height - 1)
            self.lottery = (x, y)
        else:
            if self.lottery_rect:
                self.canvas.delete(self.lottery_rect)

            self.lottery_rect = self.create_a_rect(self.lottery, color = 'gold', tag = 'food')
            return
    ...
    def hit_lottery(self):
        return self.head == self.lottery
