...
class snake_window:
    ...
    def draw_game_over(self, bypass = False):
        ...
        self.canvas.create_text(wid_1_2, hei_1_6, text = 'Game Over!',fill = 'white',font = font_set1, tag = 'gameover_text')
        self.canvas.create_text(wid_1_2, hei_2_6, text = 'Your score is : ' + str(self.score),fill = 'white', font = font_set2, tag = 'gameover_text')
...
class snake:
    ...
    def hit_walls(self):
        ...
        if y == 4 and x >= 23 and x <= 27: #110
            return True
        if (y == 3 and x >= 24 and x <= 25) : #111
            return True
        if y == 2 and x >= 29 and x <= 33: #114
            return True
        if x == 33 and y >= 2 and y <= 8: #115
            return True
        if y == 8 and x >= 31 and x <= 35: #116
            return True
